...
namespace Azteca.Helpers
{
    public class CultureHelper
    {
        ...
        public static string getCurrentNeutralCountry() {
            return GetNeutralCountry(Thread.CurrentThread.CurrentCulture.Name);
        }
        public static string GetNeutralCountry(string name)
        {
            if (!name.Contains("-")) return name;

            return name.Split('-')[1].ToLower(); // Read first part only. E.g. "AR", "ar"
        }
